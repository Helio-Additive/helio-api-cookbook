...
def workflow_thermal_histories(client):
    ...
    if not HAS_PYARROW:
        ...
        print("  but cannot be converted to CSV.")
        print("  Install with: pip install -e '.[thermal]'  (or '.[full]' for all extras)\n")
...
def workflow_plot_element_thermal():
    ...
    for element_index in elements_to_plot:
        ...
        if history:
            elem = get_element_by_index(mesh_data, element_index)
            anchor = None
            if elem and elem.get("t1") is not None and elem.get("temperature") is not None:
                anchor = (elem["t1"], elem["temperature"])
            timestamps, temperatures = extract_thermal_data(history, anchor=anchor)
...
def workflow_thermal_exploration(client):
    ...
    # Load mesh CSV so plots can be anchored at each element's extrusion temp.
    mesh_data = load_mesh_csv(mesh_path)
    ...
    while True:
        ...
        while True:
            ...
            # Extract thermal data, anchored at the deposition point if mesh data is available.
            elem = get_element_by_index(mesh_data, element_index)
            anchor = None
            if elem and elem.get("t1") is not None and elem.get("temperature") is not None:
                anchor = (elem["t1"], elem["temperature"])
            timestamps, temps = extract_thermal_data(history, anchor=anchor)
